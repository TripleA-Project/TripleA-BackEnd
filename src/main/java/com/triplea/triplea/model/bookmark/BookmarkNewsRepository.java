...
public interface BookmarkNewsRepository extends JpaRepository<BookmarkNews, Long> {
    @Query("select bn from BookmarkNews bn where bn.isDeleted=false and bn.newsId=:newsId")
    List<BookmarkNews> findByNewsId(@Param("newsId") Long newsId);

    @Query("select bn from BookmarkNews bn where bn.isDeleted=false and bn.newsId=:newsId and bn.user=:user")
    Optional<BookmarkNews> findByNewsIdAndUser(@Param("newsId") Long newsId, @Param("user") User user);
